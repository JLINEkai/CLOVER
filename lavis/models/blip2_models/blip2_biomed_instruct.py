...
from lavis.models.blip2_models.modeling_gpt2 import GPT2LMHeadModel
...
@registry.register_model("blip2_biomed_instruct")
class Blip2BiomedInstruct(Blip2Base):
    """
    BLIP2 Biomed
    """
    PRETRAINED_MODEL_CONFIG_DICT = {
        "biomed": "configs/models/blip2/blip2_instruct_biomed.yaml",
    }
    def __init__(
        self,
        ...
        freeze_vit=True,
        num_query_token=32,
        llm_model="stanford-crfm/BioMedLM",
        prompt="",
        max_txt_len=100,
        ...
        self.llm_tokenizer = GPT2Tokenizer.from_pretrained(llm_model, pad_token='<PAD>')
        self.llm_model = GPT2LMHeadModel.from_pretrained(llm_model, torch_dtype=torch.float16)
    ...
    def forward(self, samples):
        ...
        img_embeds = self.llm_proj(query_output.last_hidden_state[:,:query_tokens.size(1),:])
        ...
        # atts_llm = torch.ones(img_embeds.size()[:-1], dtype=torch.long).to(image.device)
        ...
        input_targets = input_tokens.input_ids.masked_fill(
            input_tokens.input_ids == self.llm_tokenizer.pad_token_id, -100) # bs output_txt_len(<=max_len) 0的位置填-100
        ...
        inputs_txt_embeds = self.llm_model.transformer.wte(input_tokens.input_ids) # bs txt_len 2560
        outputs_txt_embeds = self.llm_model.transformer.wte(output_tokens.input_ids) # bs txt_len 2560
    ...
    def generate(
        ...
        inputs_txt_embeds = self.llm_model.transformer.wte(input_tokens.input_ids) 
        ...
        filler_input_ids = torch.ones([inputs_embeds.shape[0],1], dtype=torch.long).to(image.device).fill_(self.llm_model.config.bos_token_id).to(image.device)
